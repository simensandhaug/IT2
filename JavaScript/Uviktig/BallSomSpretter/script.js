...
(function () {
    ...
    function init() {
        ...
        ball = {
            ...
            // velX: (Math.random() * 10 + 3) * (Math.floor(Math.random() * 2) || -1),
            // velY: (Math.random() * 15 + 6) * (Math.floor(Math.random() * 2) || -1),
            velX: 10,
            velY: 15,
            color: 'red'
        }
    ...
    function detectCharacterMovement() {
        if (keys[keys.LEFT]) {
            if(!box.x<=0) moveBox(-5);
        }
        if (keys[keys.RIGHT]) {
            if(box.x + box.width <= canvas.width-2) moveBox(5);
        }
        if (keys[keys.UP]) {
            if(!box.y<= 0)moveBox(0, -5);
        }
        if (keys[keys.DOWN]) {
            if(box.y + box.height <= canvas.height-2) moveBox(0, 5);
        }
    };
